#!/usr/bin/env python3
"""ManyIH Instruction-Following Dataset Viewer.

Generates an interactive HTML report comparing an original dataset
...
Usage:
    ...
    python -m manyih.instruction_following.viewer \
        --before before.json \
        --after after.json \
        -o diff_report.html
...
def generate_html_report(pairs, stats, before_path, after_path, output_path):
    ...
    html_content = _render_html_page(
        title="ManyIH IF Hierarchy Diff Viewer",
...
def generate_single_html_report(samples, data_path, output_path):
    ...
    html_content = _render_html_page(
        title="ManyIH IF Dataset Viewer",
...
def main():
    parser = argparse.ArgumentParser(description="ManyIH IF Dataset Viewer")
    ...
    if args.dataset and not args.before and not args.after:
        ...
        # Diff mode
        output = args.output or "manyih_if_diff_report.html"
